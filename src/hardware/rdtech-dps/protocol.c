...
static void rdtech_dps_session_send_value(const struct sr_dev_inst *sdi, struct sr_channel *ch, float value, enum sr_mq mq, enum sr_unit unit, int digits)
{
...
SR_PRIV int rdtech_dps_receive_data(int fd, int revents, void *cb_data)
{
	...
	if (sr_modbus_read_holding_registers(modbus, -1, 3, registers) == SR_OK) {
		...
		rdtech_dps_session_send_value(sdi, sdi->channels->data,
		                              RB16(registers + 0)/100.0f,
		                              SR_MQ_VOLTAGE, SR_UNIT_VOLT, 3);
		rdtech_dps_session_send_value(sdi, sdi->channels->next->data,
		                              RB16(registers + 1)/1000.0f,
		                              SR_MQ_CURRENT, SR_UNIT_AMPERE, 4);
		rdtech_dps_session_send_value(sdi, sdi->channels->next->next->data,
